...
#define DEVICE_NAME                     "BLE_Linking_LED"                           /**< Name of device. Will be included in the advertising data. */
...
#define SEC_PARAM_IO_CAPABILITIES        BLE_GAP_IO_CAPS_DISPLAY_YESNO              /**< Display Yes/No to force Numeric Comparison. */
#define SEC_PARAM_LESC                   1                                          /**< LE Secure Connections pairing required. */
#else
...
ble_pdls_result_code_t pdns_event_handler(ble_pdls_t * p_pdls, ble_pdns_event_data_t *p_pdns_event)
{
    ...
    {
      case PDNS_EVT_NOTIFY_INFO:
      {
          switch (p_pdns_event->data.notifyinfo.notifycategory)
          {
            ...
            case PDNS_NOTIFY_CATEGORY_NOTNOTIFY:
            case PDNS_NOTIFY_CATEGORY_ALL:
            case PDNS_NOTIFY_CATEGORY_PHONEINCOMINGCALL:
            case PDNS_NOTIFY_CATEGORY_PHONEINCALL:
      ...
      case PDNS_EVT_GET_PD_NOTIFY_DETAIL_DATA_RESP:
      {
      ...
      case PDNS_EVT_START_PD_APP_RESP:
      default:
        break;
   }
...
ble_pdls_result_code_t pdsos_event_handler(ble_pdls_t * p_pdls, ble_pdsos_event_data_t *p_pdsos_event)
{
    uint32_t err_code = NRF_SUCCESS;
   
    NRF_LOG_PRINTF("[LINKING] PDSOS event received: %02x\r\n", p_pdsos_event->event);
    switch (p_pdsos_event->event)
    {
      case PDSOS_EVT_GET_SETTING_INFO:
      {
        ble_pdsos_setting_info led_setting_info = { 
          .setting_id = PDSOS_SETTING_VALUE_ID_LED,
          .notify_time = PDSOS_NOTIFY_TIME_OFF, 
          .setting.led_setting.color_num        = 3,    // 3: RGB
          .setting.led_setting.color_selected   = 1,    // R
          .setting.led_setting.pattern_num      = 0,    // none
          .setting.led_setting.pattern_selected = 0     // none
        };
        err_code = ble_pdls_pdsos_get_setting_info_resp(p_pdls, PDLS_RESULT_OK, &led_setting_info);
      }
      break;
      
      case PDSOS_EVT_GET_SETTING_NAME:
      {
        char led_setting_name_str[] = {0x04, 'R', 'E', 'D', '\0',
                                       0x06, 'G', 'R', 'E', 'E', 'N', '\0',
                                       0x05, 'B', 'L' ,'U', 'E', '\0'};
        ble_pdsos_setting_name led_setting_name = {
          .setting.len = 18,
          .setting.p_val = (uint8_t *)led_setting_name_str
        };
        err_code = ble_pdls_pdsos_get_setting_name_resp(p_pdls, PDLS_RESULT_OK, &led_setting_name);
      }
      break;
      
      case PDSOS_EVT_SELECT_SETTING_INFO:
      {
        err_code = ble_pdls_pdsos_select_setting_info_resp(p_pdls, PDLS_RESULT_OK);
      }
      break;
...
static void services_init(void)
{
    ...
    //PDPIS
    init.servicelist          = PDPIS_SERVICE_BITMASK_PIS | 
                                PDPIS_SERVICE_BITMASK_NS  |
